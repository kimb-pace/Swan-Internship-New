...
balance_visits <- function(df, 
  ...
  plot_years <- df %>%
    dplyr::select(.data[[plot_col]], .data[[year_col]]) %>%
    distinct() %>%
    group_by(.data[[plot_col]]) %>%
    summarize(Years = list(sort(unique(.data[[year_col]]))), .groups = "drop")
  
  #manual override handling of selection 
  manual_df <- NULL #creates items for manual selection if used. this is the longform version of selected years 
  manual_summary <- NULL #this is the labeled version of the years for if you want to visualize/print it 
  manual_plots <- character() #names of plots hand selected, skips auto-selection for them as it sifts through 
  if (!is.null(manual_selection)) {
    manual_df <- tibble::tibble(
      !!plot_col := names(manual_selection),
      Selected_Years = manual_selection
    ) %>%
      tidyr::unnest(cols = Selected_Years) %>%
      dplyr::rename(Selected_Year = Selected_Years)
    manual_summary <- manual_df %>%
      mutate(Selection_Type = "Manual")
    manual_plots <- unique(manual_df[[plot_col]])
  }
  ...
  plot_years <- plot_years %>%
    filter(!(!!rlang::sym(plot_col) %in% manual_plots))
  ...
  #figure out what the common year combos are, so you can have it match in its auto-selection 
  #this will guide the auto-selection's decisions on the over_plots 
  common_combos <- exact_plots$Years %>%
    purrr::map_chr(~ paste(sort(.x), collapse = "_")) %>%
    table() %>%
    sort(decreasing = TRUE)
  top_combo <- strsplit(names(common_combos)[1], "_")[[1]] %>% as.numeric()
  ...
  over_plots <- over_plots %>%
    mutate(
      Selected_Years = purrr::map(Years, function(yrs) {
        combs <- combn(yrs, n_visits, simplify = FALSE)
        combs[which.min(sapply(combs, function(x) sum(abs(sort(x) - top_combo))))][[1]]}))
  ...
  summary_table <- all_selected %>%
    arrange(.data[[plot_col]], Selected_Year)
  ...
  if (nrow(under_plots) > 0) {
    cat("⚠️  Dropped", nrow(under_plots), "plot(s) with fewer than", n_visits, "visits:\n")
